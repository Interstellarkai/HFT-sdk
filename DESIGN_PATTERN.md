...
**Category:** Concurrency
**Files:** `src/market/matching_engine.hpp`, application `main.cpp`
...
```cpp
// src/orderbook/l3_order_book.hpp — private declarations
...
template std::optional<BookLevel> L3OrderBook::best_level<L3OrderBook::BidPriceLevels>(
    const BidPriceLevels&) const;
template std::optional<BookLevel> L3OrderBook::best_level<L3OrderBook::AskPriceLevels>(
...
algorithm has exactly one source of truth. The eight explicit instantiations confirm
the compiler can generate all required versions, and the linker will find them —
avoiding the classic mistake of forgetting to instantiate a template and getting
...
```cpp
// inside cancel_order()
auto idx_it = order_index_.find(id);   // O(1) hash lookup
auto& loc = idx_it->second;            // loc.it = iterator into the std::list
...
order_index_.erase(idx_it);            // O(1) erase by iterator (faster than erase by key)
```
...
class ScopeTimer {
 public:
  ...
  ~ScopeTimer();          // <- always runs, even if an exception is thrown
  void endAndLog();       // compute elapsed, print or store
};
...
// Usage:
{
  ...
  // ... measured work ...
}  // t destructor calls endAndLog() automatically here
```
...
calls `~SymbolState()` for every entry in `symbols_`, which calls each
`unique_ptr` destructor in turn.
...
makes cleanup unconditional. In HFT specifically, two consequences matter: a timer
that is never stopped contaminates latency histograms — the reject path (which is
fast) appears to have the same latency as the full match path (which is slower),
making it impossible to distinguish the two in telemetry. A `unique_ptr` that is
never freed silently exhausts the fixed-size `SymbolState` pool, causing a hard
failure at runtime with no obvious cause. RAII eliminates both classes of bug by
guaranteeing cleanup regardless of control flow.
...
The template helper returns `std::nullopt` when the book is empty:
...
// src/orderbook/l3_order_book.hpp
using TradeCallback = std::function<void(const Trade&)>;
TradeCallback trade_cb_;   // lambda, functor, or free function — doesn't matter
```

This lets completely different callers register completely different kinds of
callables with no shared base class:
...
The virtual-dispatch alternative requires every callback to inherit from a
`TradeListener` base class, implement `virtual void on_trade(const Trade&)`, and
be kept alive for the engine's lifetime. `std::function` handles all callable
types without inheritance, manages the lifetime of captured state automatically,
and avoids coupling the producer to a particular class hierarchy. The trade-off
is that `std::function` itself has a small overhead (internal type erasure
dispatch) — acceptable because callbacks fire once per order event, not once
per price-level iteration.
...
struct Symbol {
    ...
    // constructor truncates silently at 8 chars; no heap, no strlen
};
```

Passing `Symbol` by value is equivalent to passing a `uint64_t`. There is no heap
allocation, no `std::string` SSO bookkeeping.

**EngineEvent — cache-line-aligned with memcpy copy semantics:**
...
struct alignas(64) EngineEvent {
    ...
    // Non-trivially-copyable union members require explicit copy semantics:
...
copies an `EngineEvent` into a slot, that copy is `sizeof(EngineEvent)` bytes via
`memcpy` — no pointer chasing, no allocator involvement.
...
guarantee that makes the ring buffer's flat array of slots work. Fixed arrays make
the struct self-contained, stack-compatible, and `memcpy`-safe.
...
cache miss and a latency spike from the system allocator's bookkeeping. Value
semantics with fixed-size structs move all data onto the stack and into pre-allocated
ring buffer slots — zero allocator involvement on the hot path. The design mirrors
production exchange wire protocols (NASDAQ ITCH, CME MDP3) which transmit
fixed-length binary messages over UDP multicast for exactly the same reason.
