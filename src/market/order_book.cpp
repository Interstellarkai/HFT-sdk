...
void OrderBook::addOrder(const BookOrder& ord) {
  if (ord.side == Side::Buy) {
    ...
    order_index_[ord.id] =
        OrderLocation{ord.side, ord.price, std::prev(bids_[ord.price].end())};
  } else {
    asks_[ord.price].push_back(ord);
    order_index_[ord.id] =
        OrderLocation{ord.side, ord.price, std::prev(asks_[ord.price].end())};
  }
}
...
std::pair<std::vector<Trade>*, Quantity> OrderBook::matchIncoming(
