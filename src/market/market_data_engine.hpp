...
class MarketDataEngine {
 public:
  ...
  void subscribe_l1(L1Handler handler) {
    l1_handlers_.push_back(std::move(handler));
  }

  void subscribe_l2(L2Handler handler) {
    l2_handlers_.push_back(std::move(handler));
  }
  ...
  // ── Stats ──────────────────────────────────────────────────────────
  [[nodiscard]] std::uint64_t messages_published() const {
    return messages_published_;
  }

  [[nodiscard]] std::uint64_t messages_throttled() const {
    return messages_throttled_;
  }
 ...
 private:
  ...
  std::unordered_map<HFT_sdk::Symbol, HFT_sdk::Timestamp, HFT_sdk::SymbolHash>
      last_publish_time_;
