...
void TradeEngine::process_trade(const Trade& trade) {
  ...
  total_notional_ +=
      static_cast<double>(trade.qty) * static_cast<double>(trade.price);

  // Update positions for both parties
  // Incoming (aggressor)
  update_position(trade.incoming_trader, trade.symbol, trade.aggressor_side,
                  trade.qty, trade.price);

  // Resting (passive) — opposite side
  HFT_sdk::Side passive_side = (trade.aggressor_side == HFT_sdk::Side::Buy)
                                   ? HFT_sdk::Side::Sell
                                   : HFT_sdk::Side::Buy;
  update_position(trade.resting_trader, trade.symbol, passive_side, trade.qty,
                  trade.price);

  if (trade_cb_) trade_cb_(trade);
}

const Position* TradeEngine::get_position(HFT_sdk::TraderId trader_id,
                                          const HFT_sdk::Symbol& symbol) const {
...
std::vector<Position> TradeEngine::get_positions(
    HFT_sdk::TraderId trader_id) const {
...
void TradeEngine::mark_to_market(const HFT_sdk::Symbol& symbol,
                                 HFT_sdk::Price current_price) {
  for (auto& [key, pos] : positions_) {
    if (key.symbol == symbol) {
      if (pos.net_qty > 0) {
        // Long position
        pos.unrealized_pnl =
            static_cast<double>(pos.net_qty) *
...
void TradeEngine::update_position(HFT_sdk::TraderId trader_id,
                                  const HFT_sdk::Symbol& symbol,
                                  HFT_sdk::Side side, HFT_sdk::Quantity qty,
                                  HFT_sdk::Price price) {
  TraderSymbolKey key{trader_id, symbol};
  ...
  if (side == HFT_sdk::Side::Buy) {
    // If reducing a short position, realize PnL
    if (pos.net_qty < 0) {
      HFT_sdk::Quantity close_qty = std::min(qty, -pos.net_qty);
      pos.realized_pnl +=
          static_cast<double>(close_qty) * (pos.avg_sell_price - fill_price);
    }
    ...
    if (pos.net_qty >= 0) {
      ...
      pos.buy_qty += qty;
      pos.avg_buy_price =
          (old_cost + fill_price * fill_qty) / static_cast<double>(pos.buy_qty);
    }
    ...
    if (pos.net_qty > 0) {
      HFT_sdk::Quantity close_qty = std::min(qty, pos.net_qty);
      pos.realized_pnl +=
          static_cast<double>(close_qty) * (fill_price - pos.avg_buy_price);
    }
