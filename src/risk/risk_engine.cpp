...
void RiskEngine::on_fill( TraderId trader_id, const Symbol& symbol, Side side, Quantity qty, Price /*price*/ )
{
...
RejectReason RiskEngine::check_rate_limit( TraderId trader_id, Timestamp now )
{
    ...
    while ( !state.order_timestamps.empty() && ( now - state.order_timestamps.front() ) > window_ns )
    {
