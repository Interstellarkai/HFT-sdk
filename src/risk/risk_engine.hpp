...
struct TraderRiskState {
  std::unordered_map<Symbol, Quantity, SymbolHash>
      positions;                           // net position per symbol
...
class RiskEngine {
 public:
  ...
  void on_fill(TraderId trader_id, const Symbol& symbol, Side side,
  ...
  [[nodiscard]] bool is_global_kill() const {
    return global_kill_.load(std::memory_order_relaxed);
  }

  // ── Query ──────────────────────────────────────────────────────────
  [[nodiscard]] Quantity get_position(TraderId trader_id,
                                      const Symbol& symbol) const;

  [[nodiscard]] std::uint64_t orders_checked() const { return orders_checked_; }

  [[nodiscard]] std::uint64_t orders_rejected() const {
 ...
 private:
  ...
  [[nodiscard]] RejectReason check_position_limit(TraderId trader_id,
                                                  const Order& order);
  [[nodiscard]] RejectReason check_rate_limit(TraderId trader_id,
                                              Timestamp now);
  [[nodiscard]] RejectReason check_self_trade(TraderId trader_id,
