...
`clock::now()` is not free. On x86 Linux it costs roughly 20–30 nanoseconds (5–15 ns on ARM)
— it must read a hardware counter and apply calibration. This overhead wraps every measurement:
...
`clamp_index` is designed to catch this — though as you'll see in Belt and Suspenders below,
it never actually receives a dangerous value in practice:
...
The function runs the measurement loop first — when `iterations = 0`, the loop body simply
never executes (`i < 0` is false for `std::size_t`). Then, before sorting, there is a guard:
...
By the time `clamp_index` runs, `n >= 1` is guaranteed by this guard. The unsigned
...
std::println("P99:   {} ns", stats.p99_ns);   // std::println requires C++23
std::println("P99.9: {} ns", stats.p999_ns);  // substitute printf/cout for C++20
