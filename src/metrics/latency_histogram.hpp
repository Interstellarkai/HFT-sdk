...
public:
    ...
    [[nodiscard]] LatencyStats stats() const
    {
        ...
        s.min_ns    = min_ns_;
        s.max_ns    = max_ns_;
        s.mean_ns   = ( total_samples_ > 0 ) ? static_cast<std::int64_t>( sum_ns_ / static_cast<double>( total_samples_ ) ) : 0;
    ...
    [[nodiscard]] std::int64_t percentile( double p ) const
    {
        ...
        std::size_t target = static_cast<std::size_t>( std::ceil( static_cast<double>( total_samples_ ) * p ) );
        if ( target == 0 )
            target = 1;
