...
void Telemetry::record_fill(Quantity qty, Price price) {
  total_fill_qty_.fetch_add(static_cast<std::uint64_t>(qty),
                            std::memory_order_relaxed);
  total_fill_notional_.fetch_add(static_cast<std::uint64_t>(qty * price),
                                 std::memory_order_relaxed);
}
...
void Telemetry::print_dashboard() const {
  ...
  std::cout << "║   End-to-end:   " << std::setw(10) << e2e.p50_ns
            << std::setw(11) << e2e.p99_ns << std::setw(11) << e2e.p999_ns
            << "        ║\n";
  std::cout << "║   Matching:     " << std::setw(10) << me.p50_ns
            << std::setw(11) << me.p99_ns << std::setw(11) << me.p999_ns
            << "        ║\n";
  std::cout << "║   Market Data:  " << std::setw(10) << md.p50_ns
            << std::setw(11) << md.p99_ns << std::setw(11) << md.p999_ns
            << "        ║\n";
  ...
  if (fp_count > 0) {
    double avg_fp = static_cast<double>(fp_sum) /
                    (static_cast<double>(fp_count) * kFixedPointScale);
    std::cout << "║   Avg Fill P: " << std::setw(14) << std::fixed
              << std::setprecision(4) << avg_fp
              << "                          ║\n";
  }
  if (sl_count > 0) {
    double avg_sl = static_cast<double>(sl_sum) /
                    (static_cast<double>(sl_count) * kFixedPointScale);
    std::cout << "║   Avg Slip:   " << std::setw(14) << std::fixed
              << std::setprecision(4) << avg_sl
              << " ticks                    ║\n";
  }
...
void Telemetry::print_latency_summary() const {
  ...
  std::cout << "  E2E   — p50: " << e2e.p50_ns << " ns, p99: " << e2e.p99_ns
            << " ns, p999: " << e2e.p999_ns << " ns (" << e2e.samples
            << " samples)\n";
  std::cout << "  Match — p50: " << me.p50_ns << " ns, p99: " << me.p99_ns
            << " ns, p999: " << me.p999_ns << " ns (" << me.samples
            << " samples)\n";
}
