        ...
        if ( order.side == Side::Buy )
        {
            for ( auto& [price, queue] : asks_ )
            {
                if ( order.type == OrderType::Limit && price > order.price )
                    break;
                for ( auto& o : queue )
                    available += o.visible_qty + o.hidden_qty;
...
void L3OrderBook::match_against( PriceLevels& levels, const Order& incoming, Quantity& remaining, std::vector<Trade>& trades, Timestamp ts )
{
    ...
    while ( remaining > 0 && level_it != levels.end() )
    {
        ...
        {
            if ( incoming.side == Side::Buy && level_price > incoming.price )
                break;
            if ( incoming.side == Side::Sell && level_price < incoming.price )
                break;
        }
...
template void L3OrderBook::match_against<L3OrderBook::BidPriceLevels>( BidPriceLevels&, const Order&, Quantity&, std::vector<Trade>&, Timestamp );
template void L3OrderBook::match_against<L3OrderBook::AskPriceLevels>( AskPriceLevels&, const Order&, Quantity&, std::vector<Trade>&, Timestamp );
