...
class L3OrderBook {
 public:
  ...
  using DepthCallback = std::function<void(
      const HFT_sdk::Symbol&, const std::vector<HFT_sdk::BookLevel>& bids,
      const std::vector<HFT_sdk::BookLevel>& asks)>;
  explicit L3OrderBook(const HFT_sdk::Symbol& symbol);

  // ── Order operations ───────────────────────────────────────────────
  /// Submit a new order. Returns execution report.
  HFT_sdk::ExecutionReport add_order(const HFT_sdk::Order& order,
                                     HFT_sdk::Timestamp ts);

  /// Cancel an existing order.
  HFT_sdk::ExecutionReport cancel_order(HFT_sdk::OrderId id,
                                        HFT_sdk::Timestamp ts);

  /// Modify an existing order (cancel-replace).
  HFT_sdk::ExecutionReport replace_order(const HFT_sdk::ReplaceRequest& req,
                                         HFT_sdk::Timestamp ts);
  ...
  [[nodiscard]] std::vector<L3Order> orders_at_price(
      HFT_sdk::Side side, HFT_sdk::Price price) const;
  ...
  [[nodiscard]] std::size_t total_bid_orders() const {
    return bid_order_count_;
  }

  [[nodiscard]] std::size_t total_ask_orders() const {
  ...
  [[nodiscard]] HFT_sdk::Price last_trade_price() const {
 ...
 private:
  ...
  // ── Matching logic ─────────────────────────────────────────────────
  MatchResult match_incoming(const HFT_sdk::Order& order,
                             HFT_sdk::Timestamp ts);

  template <typename PriceLevels>
  void match_against(PriceLevels& levels, const HFT_sdk::Order& incoming,
