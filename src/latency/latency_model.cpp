...
void LatencyModel::set_client_profile(TraderId trader_id,
                                      const LatencyProfile& profile) {
  client_profiles_[trader_id] = profile;
}

const LatencyProfile& LatencyModel::get_client_profile(
    TraderId trader_id) const {
...
std::uint64_t LatencyModel::sample_network_latency(TraderId trader_id) {
  auto it = client_profiles_.find(trader_id);
  const auto& profile =
      (it != client_profiles_.end()) ? it->second : network_profile_;
...
std::uint64_t LatencyModel::sample_md_latency() { return sample(md_profile_); }
...
std::uint64_t LatencyModel::add_jitter(std::uint64_t base_ns,
                                       double jitter_fraction) {
  double jitter =
      normal_dist_(rng_) * static_cast<double>(base_ns) * jitter_fraction;
...
double LatencyModel::sample_gaussian_tail(const LatencyProfile& profile) {
  ...
  if (u > profile.tail_prob) {
    ...
    double tail_scale =
        (profile.p99_ns - profile.mean_ns) / 2.3;  // ~p99 of exp
