...
/// \tparam Size        Number of slots (must be power of 2). Usable capacity is
/// Size - 1.
/// \tparam WaitStrategy Compile-time policy for blocking push/pop spin
/// behavior.
...
class HPRingBuffer {
  ...
  // for efficient modulus operation, but only when Size is a power of two
 ...
 public:
  ...
  [[nodiscard]] bool try_push(const T& item) noexcept {
    ...
    buffer_[ws & mask_] = item;  // modulus (wrap around)
  ...
  [[nodiscard]] std::optional<T> try_pop() noexcept {
    ...
    T value = std::move(buffer_[rs & mask_]);  // modulus (wrap around)
  ...
  /// Number of items currently in the buffer. Branch-free with monotonic
  /// sequences.
 ...
 private:
  ...
  // sharing between the producer (write_sequence_) and consumer
  // (read_sequence_).
