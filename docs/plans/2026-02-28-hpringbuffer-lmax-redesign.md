...
**Step 1: Add GTest via FetchContent and test target to `CMakeLists.txt`**
...
```bash
git add CMakeLists.txt tests/test_hpringbuffer.cpp
git commit -m "test: set up Google Test infrastructure with HPRingBuffer smoke test"
```
...
```bash
git add tests/test_hpringbuffer.cpp
git commit -m "test: add comprehensive HPRingBuffer tests (basic ops, wraparound, wait strategies, SPSC cross-thread)"
```
